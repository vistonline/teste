		...
		if (numFilesQueued > 0)
          {
            ...
            if(this.customSettings.seguradora != 20) // SEGURADORA (20 É BONE)
            //if(true)
              {
				...
				// não redimenciona fotos desta prestadora
				if(this.customSettings.prestadora == 170){
					this.startUpload();// não redimenciona
					//this.startResizedUpload(this.getFile(0).ID, 1022, 766, SWFUpload.RESIZE_ENCODING.JPEG, 90, true);// redimenciona Para padrão 1024X768 (tirando 2 pixel apenas)
					}else{
		...
		if (this.getStats().files_queued > 0)
          {
           ...
           if(this.customSettings.seguradora != 20) // SEGURADORA 20 É BONE
              {//Não é Boné
				...
				if(this.customSettings.prestadora == 170){
					this.startUpload(); // não redimenciona  
					//this.startResizedUpload(this.getFile(0).ID, 1022, 766, SWFUpload.RESIZE_ENCODING.JPEG, 90, true);// redimenciona oara padrão 1024X768 ()tirando 2 pixels apenas
					}else{
						this.startResizedUpload(this.getFile(0).ID, this.customSettings.imageWidth, this.customSettings.imageHeight, SWFUpload.RESIZE_ENCODING.JPEG, this.customSettings.imageQuality, true);
                //alert(this.getFile(0).ID);
						}
