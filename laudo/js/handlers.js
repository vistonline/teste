function preLoad() {
	if (!this.support.loading) {
		alert("Você precisa instalar o plugin do Flash Player para fazer o carregamento das fotos.");
		return false;
	} else if (!this.support.imageResize) {
		alert("Você precisa do Flash Player 10 ou superior para redimensionar as imagens.");
		return false;
	}
}
function loadFailed() {
...
function fileQueueError(file, errorCode, message) {
	try {
		...
		if (errorCode === SWFUpload.errorCode_QUEUE_LIMIT_EXCEEDED) {
			errorName = "Você está tentando enviar muitos arquivos.";
		}
...
function uploadProgress(file, bytesLoaded) {
	...
	try {
		...
		progress.setStatus("Subindo Imagens...");
...
function uploadComplete(file) {
	try {
		...
		if (this.getStats().files_queued > 0) {
			...
			progress.setStatus("Todas as imagens foram recebidas com sucesso.");
			progress.toggleCancel(false);
		}
	} catch (ex) {
		this.debug(ex);
