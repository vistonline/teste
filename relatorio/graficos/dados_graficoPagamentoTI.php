...
$sql2 = "SELECT NRVISTORIA,DTPROC,roteirizador,pda FROM ".$bancoDados.".vistoria_previa1 WHERE DTPROC >=".$anoInicio.str_pad($mesInicio,2,"0",STR_PAD_LEFT)."01 AND DTPROC <=".$anoFim.str_pad($mesFim,2,"0",STR_PAD_LEFT)."31 ORDER BY DTPROC ASC";	
...
/*		if($codigoEmpresa=='160'){
		...
		if(substr($resultado2['DTPROC'],0,6)==$anoInicio.str_pad($mesInicio,2,"0",STR_PAD_LEFT))
			...
			if($resultado2['roteirizador']==1 && $resultado2['pda']==0){
				$contExcluida1++;
				}else{
	...
	$excluida1=(int)$contExcluida1*100;
	$realizada1=(int)$contRealizada1;
	$qtde1=(int)$contQtde1;

	$excluida2=(int)$contExcluida2*100;
	$realizada2=(int)$contRealizada2;
	$qtde2=(int)$contQtde2;

	$excluida3=(int)$contExcluida3*100;
	$realizada3=(int)$contRealizada3;
	$qtde3=(int)$contQtde3;
	
	$excluida4=(int)$contExcluida4*100;
