...
include "../../adm/conecta_maritima.php";
...
<title>Atualizar tabelas Maritima</title>
<style type="text/css">
<!--
.style1 {
	color: #000;
...
<table width="100%" border="00" cellpadding="0" cellspacing="0">
  <tr>
    <td height="50" colspan="2" style="color:#FFF; background:#000; text-align:center; font-weight:bold">&nbsp;&nbsp;Importar Arquivos - Atualiza&ccedil;&atilde;o de Tabelas Maritima</td>
  </tr>
  <tr>
...
preg_match('/(Maritima_)[^|]{1,}/i', $_FILES['Filedata']["name"], $result);
...
move_uploaded_file(strtr($_FILES['Filedata']['tmp_name'],array("À"=>"A","Â"=>"A","Ê"=>"E","Ô"=>"O","Û"=>"U","Ã"=>"A","Õ"=>"O","Á"=>"A","É"=>"E","Í"=>"I","Ó"=>"O","Ú"=>"U","Ç"=>"C","Ü"=>"U","&"=>"e","´"=>"","'"=>"","("=>"",")"=>"","{"=>"","}"=>"","["=>"","]"=>"","ª"=>"","º"=>"","°"=>"","à"=>"A","â"=>"A","ê"=>"E","ô"=>"O","û"=>"U","ã"=>"A","õ"=>"O","á"=>"A","é"=>"E","í"=>"I","ó"=>"O","ú"=>"U","ç"=>"C","ü"=>"U"," "=>"_")), $arquivo);
$fp = fopen($arquivo,'r');

if( ($nomeArquivo=='Maritima_') && (strtoupper($extensao)=='CSV') )
{
...
unlink($arquivo);
	...
	$arquivo = "atualizacoesTemp/atualizarTabelasMaritima - ".$_SESSION['roteirizador'] . " - hora - ".date('H:i')." - " . $_FILES['Filedata']['name'];
...
$registra->upload($_SERVER['REMOTE_ADDR'],"relatorio/","atualizarTabelasMaritima.php",$_FILES['Filedata']["name"],$_SESSION['id'],$_SESSION['permissao'],$obs );
}
